...
class SearchEngine:
    ...
    def evaluateDataset(self):
        ...
        for k in range(1, 11):
            ...
            p.append(ps)
            recall, rs = self.evaluator.meanRecall(doc_IDs_ordered, query_ids, qrels, k)
            recalls.append(recall)
            r.append(rs)
            fscore, fs = self.evaluator.meanFscore(doc_IDs_ordered, query_ids, qrels, k)
            ...
            MAPs.append(MAP)
            m.append(ms)
            nDCG, ns = self.evaluator.meanNDCG(doc_IDs_ordered, query_ids, qrels, k)
            nDCGs.append(nDCG)
            n.append(ns)
    ...
    def handleCustomQuery(self):
        ...
        # Running spellcheck on the queries
        check = SpellChecker()
        corrected_query = ""
